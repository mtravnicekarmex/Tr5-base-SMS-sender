...
## 2026-08-18T11:49:50+02:00 — IMPLEMENTATION_CONTRACT_0002

Architecture review finding (IMPLEMENTATION_CONTRACT_0002, migrating source/project/ into project/, 2026-08-18): a verbatim-copy migration contract that lists specific source filenames as Outputs needs an explicit check against AGENTS.md's naming convention (no hyphens) for any of those filenames once they land in a governed directory (project/) — a name that was fine as read-only reference under source/ is not automatically fine once copied into project/. Separately, when a contract's acceptance criteria include actually running a migrated test suite, verify the contract also specifies how the runtime dependencies (per the migrated pyproject.toml) get installed — the root requirements.txt only covers the framework's own dependencies, not a migrated application's. Future architecture reviews of migration contracts should check both points explicitly.

## 2026-08-18T12:05:37+02:00 — IMPLEMENTATION_CONTRACT_0002

Reviewer finding (IMPLEMENTATION_CONTRACT_0002, architecture review): the `programmer` agent's permission_profile 'edit' (Claude provider) maps via agents/agent.py's CLAUDE_EDIT_TOOLS to Read/Grep/Glob/Edit/Write only — no Bash tool. `reviewer`/`architect` both use permission_profile 'review' (Read/Grep/Glob only), and no agent in the pipeline currently runs with 'full' (the only profile that grants Bash). A future contract point that asks the programmer to run a shell/git/pip command (venv creation, `pip install`, `python -m unittest` from the CLI, `git status`/`git diff`/`git check-ignore`) is not executable by the programmer as written — such verification needs to be scoped as a manual/owner step outside the automated contract points, or the architect needs to confirm a permission_profile change, not assumed achievable just because it sounds like 'run the tests'.

## 2026-08-18T15:15:04+02:00 — IMPLEMENTATION_CONTRACT_0003

IMPLEMENTATION_CONTRACT_0003 established a reusable pattern for protecting a shared-file write path with no new dependency: an exclusive sidecar '<target file>.lock' created via open(path, "x") as the very first filesystem interaction (before any read of the target), guaranteed removal via try/finally scoped to only the code that actually created the lock (so a rejecting call never deletes a lock it didn't create), and rejection surfaced as an existing error type (SpreadsheetError) the caller already handles. Reviewed and approved in full; verified by reading project/send_sms.py and project/tests/test_send_sms.py directly. Stale-lock recovery (crash-left-behind lock) was deliberately deferred, not solved, and is documented as a residual limitation in project/README.md.
